...
variable "node_count_additional" {}
variable "disk_size_gb_additional" {}
variable "machine_type_additional" {}
...
resource "google_container_node_pool" "airflow" {
  project     = "${local.project_id}"
  name_prefix = "airflow-"
  cluster     = "${element(split("/", google_composer_environment.etl_airflow.config.0.gke_cluster),11)}"
